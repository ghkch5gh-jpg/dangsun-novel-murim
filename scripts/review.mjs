#!/usr/bin/env node
// 편집자 리뷰(무협) — 최근 회차 + 캐논을 읽고 발전 리뷰 + 다음 화 생성 지침을 review.md 로 저장.
// build-local.mjs 가 review.md 의 '다음 화 생성 지침'을 읽어 다음 생성에 반영(자가발전 루프).
//   사용: node scripts/review.mjs   (옵션: REVIEW_LAST=10)
...
당신은 한국 무협 웹소설 베테랑 편집자입니다. 아래 선협·신화 무협 연재작의 **최근 ${recent.length}개 회차**(현재 ${total}/100화)와 **캐논**을 읽고, 작품을 *발전*시킬 냉정하고 구체적인 편집 리뷰를 쓰세요. 칭찬용 무난한 말 금지. 실제 본문의 문장·전개를 근거로.
...
## 약점·위험
- (3~5개. 어느 화/어떤 전개가 왜 문제인지)

## 플롯홀·논리오류
- (있으면 어느 화. 없으면 "특이사항 없음")

## 반복·매너리즘
- (표현·전개·구조의 반복 패턴)

## 캐릭터
- (묵야·핵심 조연의 매력/일관성/평면성)

## 사실·캐논 오류 (소프트 모순 포함)
- (캐논·내적 논리와 어긋난 것들. 어느 화)

## 다음 ${Math.min(10, 100 - total)}화 방향 제안
- (큰 그림. 2~4개)

## 다음 화 생성 지침
(생성기에 바로 먹일 명령형 지침 5~8개. 짧고 실행가능하게.)
- `;
...
console.log(`[무협] 리뷰: 최근 ${recent.length}화 (${total}/100) · Codex ${CODEX_MODEL || "default"}/${CODEX_REASONING_EFFORT}`);
