...
const STYLE = `# 양산형 선협·신화 무협 문법 — 대중성·재미 최우선 (반드시 준수)
- **첫 3줄 후킹**: 풍경·설명으로 느리게 시작 절대 금지. 첫 문장부터 포식/사패성/위기/한 방으로 멱살 잡기.
- **사이다 ≥ 1 (직접적·시각적)**: 매 화 반드시, [[묵야]]가 강자(신·요괴·고수)를 *눈앞에서* 압도하거나 먹어 도약하는 통쾌함. 상대의 경악·말문 막힘·역전이 보이게.
- **절단신공**: 마지막 1~2문장은 강한 훅/반전/위기로 끊기.
- **문체(대중성 핵심)**: 문장 짧고 단순하게. **대사·행동 비중 아주 높게**(가능하면 본문 절반 이상 대사). 한 문단 2~4문장. 무협 결(초식·기·살기)은 한두 단어로만 찍고, 묘사·만연체·미문 최소 — 폰으로 빠르게 읽혀야 함. 사패성(차갑고 가차없음) 유지.
- **속도**: 한 화 = 명확한 사건 하나 + 한 방. 늘어지는 내면묘사 금지.
- **차별점(THE HOOK)**: '신을 먹는 식신자 + 포식의 대가(굶주림)'를 매 화 살려라. 정파 협객 클리셰로 회귀 금지.
- 분량: 본문 **1800~2600자** (짧게! 늘이지 말 것). 시점: [[묵야]] 밀착.
...
\`\`\`
{
  ...
  "body_md": "본문 마크다운 1800~2600자, 절단신공으로 끝",
